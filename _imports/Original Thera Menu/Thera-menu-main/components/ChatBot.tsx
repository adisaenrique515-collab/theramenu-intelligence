...
import { chatWithDietitian } from '../services/geminiService';

const ChatBot: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState<{ role: 'user' | 'bot'; text: string }[]>([
    { role: 'bot', text: 'Hello! I am your TheraMenu AI Dietitian. How can I help you with clinical dietary protocols today?' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', text: 'I encountered an error. Please check your API configuration.' }]);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="absolute bottom-20 right-0 w-[350px] sm:w-[400px] h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 duration-300">
          ...
          <div className="bg-slate-900 p-4 text-white flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-sm font-semibold">AI Clinical Dietitian</h3>
                <p className="text-[10px] text-slate-400 font-mono uppercase tracking-widest">TheraMenu Intelligence</p>
              </div>
            </div>
