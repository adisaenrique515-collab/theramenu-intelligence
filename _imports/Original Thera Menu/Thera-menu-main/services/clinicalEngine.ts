...
function kcalTarget(profile: PatientProfile): number {
  const base = profile.sex === 'male' ? 2000 : 1800;
  const ageAdjustment = profile.age > 60 ? -150 : profile.age < 25 ? 100 : 0;
  const goalAdjustment = profile.goal === 'weight_loss' ? -300 : profile.goal === 'weight_gain' ? 300 : 0;
  return Math.max(1200, base + ageAdjustment + goalAdjustment);
}
...
function applySchemaTargets(profile: PatientProfile, targets: ClinicalTargets, config: ProtocolConfig): ClinicalTargets {
  const energy = kcalTarget(profile);
  const proteinMin = Math.round(config.targets.protein_g_per_kg_min * profile.weightKg);
...
export function generateWeeklyPlan(profile: PatientProfile): WeeklyTherapeuticPlan {
  ...
  const preliminaryPlan: WeeklyTherapeuticPlan = {
    ...
    constraints: {
      ...
      nutrientTargets: `${resolvedTargets.caloriesKcal} kcal/day target; Na <= ${resolvedTargets.sodiumMgMax} mg`,
    },
    days,
    ...
    notes: [
      ...
      'Generated by deterministic clinical engine',
    ...
    rationale: legacyRule?.therapeutic_goal || fallbackRationale,
    preparedBy: 'Clinical Engine (Schema-Enriched)',
    updatedDate: new Date().toISOString().slice(0, 10),
    compoundDietCodes: [resolvedTexture.toUpperCase(), profile.diagnosis],
  };
