// scripts/usda/ingest.mjs
import fs from 'fs';
import path from 'path';
import xlsx from 'xlsx';

const OUT_DIR = path.resolve('./public/usda');
const SRC = {
...
function readNutrients(file){
  ...
  for(const r of rows){
    ...
    if(!map.has(key)) map.set(key,{ name, norm:key, per_100g:{} });
    const rec = map.get(key).per_100g;
    switch(ndesc){
  ...
  case 'Potassium': rec.potassium_mg = val; break;
  case 'Phosphorus': rec.phosphorus_mg = val; break;
...
async function run(){
  ...
  const nutrients = readNutrients(SRC.fnddsNutrients);
  const portionsMap = readPortions(SRC.fnddsPortions);
  ...
  console.log(`[ingest] Wrote ${merged.length} items to public/usda/fndds_ingredients.json`);
}
