export const getMockRecipeData = (mimeType) => {
  ...
  return {
    ...
    analysis: {
      structuralStability: { fsdi: 0.82, acidBalance: 0.55, fragilityRisk: 'Medium' },
      yieldConversion: { rawToTrimCookRatio: '100?95?90%', portionScalingFactor: 1.0, costFactor: 0.35, portionMultiplier: 1.1 },
      costIntelligence: { costPerPortion: Number(costPerPortion.toFixed(2)), contributionMargin: Number((proposedPrice - costPerPortion).toFixed(2)), foodCostPercentage: Number(((costPerPortion / proposedPrice) * 100).toFixed(2)), potentialVsActualVariance: 'Within ±2%', breakEvenPointCovers: 120 },
      inventoryIntelligence: { recommendedTurnoverRatio: 4.5, targetAverageAgeDays: 6, parLevelGuidance: 'Maintain 2× daily usage for key perishables' },
      menuEngineering: { classification: 'Star', popularityIndex: 74, contributionRank: 1 },
      procurementForecast: 'Order ~10kg chicken and 6kg cheese weekly at current volumes.',
      controlProtocols: ['Blind taste test for sauce weekly','FIFO rotation','Butcher test for yield','Oil change schedule']
    }
  };
};
