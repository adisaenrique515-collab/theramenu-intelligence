...
function estimateFluidFromName(name: string, grams: number): number {
  const normalized = name.toLowerCase();
  if (normalized.includes('water') || normalized.includes('juice') || normalized.includes('tea') || normalized.includes('milk')) {
    return grams;
  }
  if (normalized.includes('yogurt') || normalized.includes('porridge') || normalized.includes('oatmeal')) {
    return grams * 0.8;
  }
  if (normalized.includes('apple') || normalized.includes('pear') || normalized.includes('fruit') || normalized.includes('vegetable') || normalized.includes('broccoli') || normalized.includes('bean')) {
    return grams * 0.75;
  }
  return grams * 0.45;
}
  ...
  meals.forEach((meal) => {
    meal.slots?.forEach((slot) => {
      ...
      if (item.nutrientProfile) {
        ...
        if (!fallbackFood) {
          totals.fluidMl += estimateFluidFromName(item.name, grams);
        }
        return;
      }
...
export function validateWeeklyPlan(plan: WeeklyTherapeuticPlan): PlanValidationReport {
  ...
  plan.days.forEach((day) => {
    ...
    if (typeof day.dailyTargets.proteinGMax === 'number') {
      nutrientChecksTotal += 1;
      if (totals.proteinG <= day.dailyTargets.proteinGMax) {
        nutrientChecksPassed += 1;
      } else {
        issues.push(
          `${day.dayName}: protein exceeded (${totals.proteinG}g > ${day.dailyTargets.proteinGMax}g).`,
        );
      }
    }
    ...
    day.meals.forEach((meal) => {
      ...
      meal.slots?.forEach((slot) => {
        ...
        const weeklyKey = item.foodId || (item.name ? item.name.toLowerCase() : '');
        if (weeklyKey && !(item.name ?? '').toLowerCase().includes('water')) {
          weeklyFoodCounts.set(weeklyKey, (weeklyFoodCounts.get(weeklyKey) || 0) + 1);
        }
